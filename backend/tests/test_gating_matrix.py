...
def test_free_tier_cannot_save_opus(_paper, _cap, _daily, _tier):
    ...
    assert enforce_model("free-user", "claude-opus-5") == "mistral-small-latest"


@patch("app.gating.get_user_tier", return_value="scholar")
def test_scholar_downgrade_keeps_anthropic(_tier):
    assert enforce_model("scholar-user", "claude-opus-4-7") == "claude-sonnet-5"
    assert enforce_model("scholar-user", "claude-opus-5") == "claude-sonnet-5"
...
def test_free_user_sonnet_model_tier_locked(_tier):
    with pytest.raises(HTTPException) as exc:
        reserve_usage("free-user", "paper-x", "qa", model="claude-sonnet-5")
