...
function DashboardContent() {
  ...
  // PDF, instead of letting a doomed multi-MB upload proceed to the backend.
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (fileRejections) => {
      ...
      if (first.code === "file-too-large") {
        setError(`PDF is too large. Max ${(MAX_UPLOAD_BYTES / 1024 / 1024).toFixed(0)} MB.`);
      } else if (first.code === "file-invalid-type") {
        setError("Only PDF files are supported.");
      } else {
        setError(first.message || "Upload rejected.");
      }
    },
      ...
      <main className="flex-1 flex flex-col items-center px-6 pt-[15vh] pb-12 bg-mesh min-h-screen bg-background text-foreground">
        ...
        <div className="max-w-[400px] w-full space-y-10 text-center">
          ...
          <div className="glass rounded-2xl py-12 px-6 space-y-4">
            ...
            <p className="text-[16px] font-semibold text-foreground">Coming soon to mobile</p>
            ...
            {loading ? (
              ...
              <div className="text-center space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-muted-foreground mt-0.5">arXiv, Nature, Science, PRL...</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3 animate-fade-in">
            ...
            <div className="glass rounded-2xl divide-y divide-border overflow-hidden">
              {papers.slice(0, 3).map((p) => (
                ...
                >
                  <p className="text-[14px] truncate font-medium text-foreground leading-snug group-hover:text-foreground">{p.title || `paper-${p.id}`}</p>
