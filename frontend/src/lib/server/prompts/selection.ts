...
- JSON-string LaTeX: every backslash inside a JSON string MUST be doubled. Write "\\\\hat{H}" so JSON parses to "\\hat{H}". A SINGLE backslash before a non-escape char (\\h, \\v, \\m, \\f, \\e, \\b…) is dropped by JSON parsers and the LaTeX command is destroyed. Example correct: "$$\\\\hat{H} = \\\\sum_{n\\\\mathbf{k}} \\\\varepsilon_{n\\\\mathbf{k}}$$". Example WRONG: "$$\\hat{H} = \\sum_{n\\mathbf{k}} \\varepsilon_{n\\mathbf{k}}$$".
  ...
  if (action === "explain") {
    return {
      system: [
        ...
        `- "action" = "explain"`,
        `- "body": a clear markdown explanation in under ~200 words unless the passage is unusually dense. If the selection ends like a question, ANSWER it directly using the paper as context. If it is a statement, EXPLAIN it: break down jargon, clarify the logic, give context and implications. Use math delimiters where helpful.`,
        `- "assumptions": ONLY premises THIS excerpt explicitly states or unmistakably depends on. Max 4 entries; empty array is fine. Each entry has "type" ("explicit"|"implicit"), "statement", and optional "significance".`,
  ...
  if (action === "derive") {
    return {
      system: [
        ...
        `- "steps": 4–8 atomic steps. Each step has step_number (1-indexed), prompt (instruction), answer (resulting expression or stated inference, markdown with math delimiters), explanation (why this step follows; 1–2 sentences), and an optional hint.`,
