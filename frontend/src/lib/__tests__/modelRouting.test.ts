...
describe("model routing", () => {
  it("canonicalizes retired Anthropic 4.x ids", () => {
    ...
    expect(normalizeModelSlug("claude-opus-4-7")).toBe("claude-fable-5");
    expect(normalizeModelSlug("claude-opus-5")).toBe("claude-fable-5");
  });
  ...
  it("maps first-party slugs to AI Gateway catalog ids", () => {
    ...
    expect(toGatewayModelId("gpt-5-mini")).toBe("openai/gpt-5.4-mini");
    expect(toGatewayModelId("gpt-5.6-sol")).toBe("openai/gpt-5.6-sol");
  });
